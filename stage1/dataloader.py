...
class SwissAD_Dataset():
    def __init__(self,
        ...
        for anno_idx in anno_indices:
            ...
            # Video path
            filename = clip_df.iloc[0]['movie_title']
            ...
            for shot_idx, shot_row in clip_df.iterrows():
                ...
                shot_labels.append(shot_row["shot_label"])
                shot_scales.append(shot_row["shot_scale"])
            self.all_clips.append([imdbid, anno_idx, filename, video_path, threads, text_gt, gt_start, gt_end, shot_starts, shot_ends, shot_bboxes, shot_pred_ids, shot_labels, shot_scales])
        print(f"In total {len(self.all_clips)} SwissAD clips")

    def __len__(self):
        return len(self.all_clips)

    def __getitem__(self, index):
        imdbid, anno_idx, filename, video_path, threads, text_gt, gt_start, gt_end, shot_starts, shot_ends, shot_bboxes, shot_pred_ids, shot_labels, shot_scales = self.all_clips[index]
